...
aws_data = pricing_data.get('aws_lambda')
...
aws_lambda_gib_pp_s = float(aws_data['gib_price_per_second'])
...
mins = st.slider('Minutes', 1, 480, 60)
num_tasks = st.slider('Number of Tasks', 1, 1000, 100)
...
aws_lambda_usd_value = 8 *  aws_lambda_gib_pp_s  * total_secs
...
# Pie chart data
labels = 'AZ container apps', 'GKE Autopilot', 'AWS Lambda', 'GCP Cloud Run', 'GCP Cloud Functions'
sizes = [azure_usd_value, gke_usd_value, aws_lambda_usd_value, cloudrun_usd_value, cloud_func_usd_value]
